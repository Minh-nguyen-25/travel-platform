        ...
        <article className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:border-primary-100 hover:shadow-md">
          <div className="flex flex-col sm:flex-row">
            ...
            <div className="min-w-0 flex-1 p-5">
              ...
              {itinerary.destination.categories.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{itinerary.destination.categories.slice(0, 3).map((category) => <span key={category.id} className="rounded-md bg-gray-100 px-2 py-1 text-[10px] font-bold uppercase tracking-normal text-gray-500">{category.name}</span>)}</div>}
      ...
      <main className="container relative -mt-9">
        ...
        <div className="mt-8 grid gap-7 lg:grid-cols-[220px_minmax(0,1fr)_260px]">
          <aside className="trip-print-hide lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm"><p className="px-2 pb-2 pt-1 text-[10px] font-extrabold uppercase tracking-normal text-gray-400">Các ngày</p><div className="flex gap-2 overflow-x-auto lg:flex-col">{trip.tripDays.map((day) => <button type="button" key={day.dayNumber} onClick={() => scrollToDay(day.dayNumber)} className={`min-w-[130px] rounded-xl px-3 py-3 text-left transition lg:min-w-0 ${activeDay === day.dayNumber ? 'bg-primary-600 text-white shadow-md shadow-primary-200' : 'text-gray-600 hover:bg-primary-50 hover:text-primary-700'}`}><span className={`block text-[10px] font-extrabold uppercase tracking-normal ${activeDay === day.dayNumber ? 'text-primary-100' : 'text-gray-400'}`}>Ngày {day.dayNumber}</span><span className="mt-1 block text-xs font-bold capitalize">{formatDate(day.date, { weekday: 'short', day: '2-digit', month: '2-digit' })}</span><span className={`mt-1 block text-[10px] ${activeDay === day.dayNumber ? 'text-primary-100' : 'text-gray-400'}`}>{day.itineraries.length} điểm dừng</span></button>)}</div></div>
          </aside>
          ...
          <div className="min-w-0 space-y-10">
            {trip.tripDays.map((day) => (
              <section key={day.dayNumber} id={`shared-day-${day.dayNumber}`} className="trip-print-section scroll-mt-28">
                <div className="mb-5"><p className="text-xs font-extrabold uppercase tracking-normal text-primary-600">Ngày {day.dayNumber}</p><h2 className="mt-1 text-2xl font-extrabold capitalize text-gray-900">{formatDate(day.date, { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}</h2>{day.note && <p className="mt-2 text-sm text-gray-500">{day.note}</p>}</div>
