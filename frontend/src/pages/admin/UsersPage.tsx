                  ...
                  {users.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="transition hover:bg-slate-50/70">
                        ...
                        <td className="px-5 py-4"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${user.authProvider === 'GOOGLE' ? 'bg-red-50 text-red-600' : user.authProvider === 'FACEBOOK' ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600'}`}><span className="h-1.5 w-1.5 rounded-full bg-current" />{user.authProvider === 'GOOGLE' ? 'Google' : user.authProvider === 'FACEBOOK' ? 'Facebook' : 'Email'}</span></td>
