  ...
  return (
    ...
    >
      <div className="container flex h-full items-center justify-between gap-4">
        <Link
          to={ROUTES.HOME}
          className={`group flex flex-none items-center gap-2.5 ${transparent ? 'text-white hover:text-white' : 'text-navy-900 hover:text-navy-900'}`}
          aria-label="TravelPlatform - Trang chủ"
        >
          <span className={`relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-2xl border shadow-lg transition duration-300 group-hover:-rotate-3 group-hover:scale-105 ${
            transparent
              ? 'border-white/25 bg-white/12 text-white backdrop-blur-md'
              : 'border-primary-200 bg-gradient-to-br from-primary-500 to-primary-800 text-white shadow-primary-900/15'
          }`}>
            <TripIcon name="compass" size={21} strokeWidth={2.2} />
            <span className="absolute -bottom-2 -right-2 h-4 w-4 rounded-full bg-accent-400" />
          </span>
          <span className="text-base font-extrabold tracking-[-0.035em] sm:text-lg">
            Travel<span className={transparent ? 'text-primary-200' : 'text-primary-600'}>Platform</span>
          </span>
        </Link>
