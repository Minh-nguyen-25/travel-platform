...
async function main() {
  ...
  const coordinatePlan = planVerifiedCoordinates(coordinateDocument, destinations.filter(item => !item.coordinateSourceUrl));
  ...
  const photoPlan = planDestinationPhotos(photoDocument, photoRecords);
