...
async function seedSampleTrip(userId, input) {
    ...
    const tripData = {
        ...
        startDate,
        endDate,
        budget: new client_1.Prisma.Decimal(input.budget),
    ...
    await prisma.$transaction(Array.from({ length: totalDays }, (_, index) => prisma.tripDay.upsert({
        where: {
            tripId_dayNumber: {
                tripId: trip.id,
                dayNumber: index + 1,
...
async function seedSampleDestination(input) {
    ...
    const destinationData = {
        ...
        rating: new client_1.Prisma.Decimal(input.rating),
        isActive: true,
    };
    ...
    await prisma.$transaction([
        ...
        ...input.categoryIds.map((categoryId) => prisma.destinationCategory.upsert({
            where: {
...
async function main() {
    ...
    for (const destination of destinations.slice(0, 3)) {
        ...
        await prisma.destination.update({
            ...
            data: { rating: new client_1.Prisma.Decimal(averageRating) },
        });
    }
    ...
    for (const [index, tripDay] of firstTripDays.entries()) {
        ...
        await prisma.itinerary.upsert({
            ...
            update: {
                ...
                estimatedCost: new client_1.Prisma.Decimal(100_000),
                travelMode: 'DRIVING',
            },
            create: {
                tripDayId: tripDay.id,
