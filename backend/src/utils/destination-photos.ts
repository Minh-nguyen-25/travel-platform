...
// These are the five generic images used by the original seed, not user uploads.
...
export function planDestinationPhotos(document: unknown, records: DestinationRecord[]) {
  ...
  for (const photo of photos) {
    const matches = records.filter(record => record.name === photo.name);
    if (matches.length !== 1) {
      skipped.push({ name: photo.name, reason: matches.length ? 'ambiguous-name' : 'not-in-database' });
      continue;
    }
    const destination = matches[0];
    if (destination.images.some(image => image.imageUrl === photo.imageUrl)) {
