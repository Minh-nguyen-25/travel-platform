...
const optionalNumber = (schema: z.ZodType<number>) =>
  z.preprocess((value) => (value === '' || value === null ? undefined : value), z.coerce.number().pipe(schema).optional());
...
const optionalNullableText = (max: number, label: string) =>
  z.preprocess(
    (value) => (value === '' ? null : value),
    z.string().trim().max(max, `${label} không được vượt quá ${max} ký tự`).nullable().optional()
  );

const parseIdList = (value: unknown): unknown => {
...
const moneySchema = z
  ...
  .refine(
    ...
    'Giá vé chỉ được có tối đa 2 chữ số thập phân'
  );
...
const ratingSchema = z
  ...
  .refine(
    ...
    'Điểm đánh giá chỉ được có tối đa 1 chữ số thập phân'
  );

const destinationFields = {
  name: z.string().trim().min(1, 'Tên địa điểm là bắt buộc').max(200),
  description: optionalNullableText(20_000, 'Mô tả'),
  address: z.string().trim().min(1, 'Địa chỉ là bắt buộc').max(2_000),
...
const primaryImageIndexSchema = z.preprocess(
...
const baseDestinationListQuerySchema = z
  .object({
    page: z.coerce.number().int().min(1).default(1),
    limit: z.coerce.number().int().min(1).max(100).default(12),
    search: z.string().trim().min(1).max(200).optional(),
    categoryId: positiveIdSchema.optional(),
    categoryIds: z.preprocess(parseIdList, z.array(positiveIdSchema).max(20).optional()),
    ...
    sortBy: z.enum(['createdAt', 'name', 'rating', 'ticketPrice']).default('createdAt'),
    sortOrder: z.enum(['asc', 'desc']).default('desc'),
  })
  .strict()
  .superRefine((data, context) => {
    if (data.minPrice !== undefined && data.maxPrice !== undefined && data.minPrice > data.maxPrice) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['maxPrice'], message: 'maxPrice phải lớn hơn hoặc bằng minPrice' });
    }
    if (data.minRating !== undefined && data.maxRating !== undefined && data.minRating > data.maxRating) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['maxRating'], message: 'maxRating phải lớn hơn hoặc bằng minRating' });
    }
  })
...
export const destinationListQuerySchema = baseDestinationListQuerySchema;

export const adminDestinationListQuerySchema = z.preprocess(
  (value) => value,
  z
    .object({
      page: z.coerce.number().int().min(1).default(1),
...
export const destinationIdParamsSchema = z.object({ destinationId: positiveIdSchema }).strict();

export const destinationImageParamsSchema = z
  .object({ destinationId: positiveIdSchema, imageId: positiveIdSchema })
  .strict();

export const uploadDestinationImagesSchema = z.object({ primaryImageIndex: primaryImageIndexSchema }).strict();
...
export const categoryIdParamsSchema = z.object({ categoryId: positiveIdSchema }).strict();
