import { z } from 'zod';
import { BUDGET_LEVEL } from '../constants';

const MAX_PREFERENCE_ITEMS = 50;
const MAX_PREFERENCE_ITEM_LENGTH = 100;

const preferenceListSchema = (label: string) =>
  z
    .array(
      z
        ...
        .max(
          MAX_PREFERENCE_ITEM_LENGTH,
          `${label} không được vượt quá ${MAX_PREFERENCE_ITEM_LENGTH} ký tự`
        )
    )
    ...
    .refine(
      ...
      `${label} không được chứa giá trị trùng lặp`
    );

const preferenceFields = {
  budgetLevel: z
    .enum([BUDGET_LEVEL.LOW, BUDGET_LEVEL.MEDIUM, BUDGET_LEVEL.HIGH], {
...
export const createPreferenceSchema = z.object(preferenceFields).strict();
