...
test('access and refresh JWTs have distinct types; refresh tokens are hashed before persistence', () => {
  const accessToken = generateAccessToken({ userId: 7, email: 'user@example.com', role: 'USER' });
  const refresh = generateRefreshToken({
    userId: 7,
    tokenId: 'fdd1f5d5-d699-4c14-8548-dfdb6c8f58cc',
  });

  assert.equal(verifyAccessToken(accessToken).type, 'access');
  assert.equal(verifyRefreshToken(refresh.token).type, 'refresh');
  assert.throws(() => verifyAccessToken(refresh.token));
  ...
  assert.ok(refresh.expiresAt.getTime() > Date.now());
});

test('auth cookies are HttpOnly/Secure in production and can be parsed from requests', () => {
  ...
  setRefreshTokenCookie(response, 'refresh-token-value', new Date(Date.now() + 60_000));
  ...
  assert.equal(captured.options.path, '/api/v1/auth');
  assert.equal(
    getRefreshTokenFromRequest({
      headers: { cookie: `other=1; ${REFRESH_COOKIE_NAME}=refresh-token-value` },
    }),
    'refresh-token-value'
  );

  setGoogleOAuthStateCookie(response, 'oauth-state-value');
  assert.equal(captured.name, GOOGLE_OAUTH_STATE_COOKIE_NAME);
  assert.equal(captured.options.httpOnly, true);
  assert.equal(captured.options.secure, true);
  assert.equal(captured.options.sameSite, 'lax');
  assert.equal(captured.options.path, '/api/v1/auth/google/callback');
  assert.equal(captured.options.maxAge, 10 * 60 * 1000);
  assert.equal(
    getGoogleOAuthStateFromRequest({
      headers: { cookie: `${GOOGLE_OAUTH_STATE_COOKIE_NAME}=oauth-state-value` },
    }),
    'oauth-state-value'
  );
