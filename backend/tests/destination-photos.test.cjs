...
test('target seed placeholder is replaced and dash variants match the same landmark', () => {
  const photo = { ...manifest.photos[0], slug: 'van-mieu', name: 'Văn Miếu - Quốc Tử Giám',
    imageUrl: '/images/destinations/van-mieu.jpg' };
  const plan = utility.planDestinationPhotos({ ...manifest, photos: [photo] }, [{
    id: 9, name: 'Văn Miếu – Quốc Tử Giám', images: [{
      id: 7, imageUrl: '/images/destinations/placeholder.svg', isPrimary: true, displayOrder: 0,
    }],
  }]);
  assert.deepEqual(plan.operations, [{ destinationId: 9, imageId: 7,
    imageUrl: '/images/destinations/van-mieu.jpg', isPrimary: true, displayOrder: 0 }]);
});

test('a prior secondary local photo becomes primary when the seed placeholder is still primary', () => {
  const plan = utility.planDestinationPhotos(manifest, [{ id: 2, name: 'Hồ Hoàn Kiếm',
    images: [
      { id: 1, imageUrl: '/images/destinations/placeholder.svg', isPrimary: true, displayOrder: 0 },
      { id: 2, imageUrl: manifest.photos[0].imageUrl, isPrimary: false, displayOrder: 1 },
    ] }]);
  assert.deepEqual(plan.operations, [
    { destinationId: 2, imageId: 1, imageUrl: '/images/destinations/placeholder.svg', isPrimary: false, displayOrder: 0 },
    { destinationId: 2, imageId: 2, imageUrl: manifest.photos[0].imageUrl, isPrimary: true, displayOrder: 1 },
  ]);
});
