...
test('verified coordinates match the target seed dash variant without changing the source', () => {
  const document = { ...sample, destinations: [{ ...sample.destinations[0],
    name: 'Văn Miếu - Quốc Tử Giám', latitude: 21.0282, longitude: 105.8356 }] };
  const plan = utility.planVerifiedCoordinates(document, [{ id: 9,
    name: 'Văn Miếu – Quốc Tử Giám', latitude: '21.0280430', longitude: '105.8354820',
  }], new Date('2026-09-15T12:00:00Z'));
  assert.equal(plan.updates.length, 1);
  assert.equal(plan.updates[0].id, 9);
  assert.equal(plan.updates[0].coordinateSourceUrl, sample.destinations[0].sourceUrl);
});
